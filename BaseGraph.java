...
public class BaseGraph<NodeType, EdgeType extends Number> {
    ...
    protected Edge getEdgeHelper(NodeType pred, NodeType succ) {
        System.out.println("getEdgeHelper method called with nodes: " + pred.toString() + " and " + succ.toString());
        // Check if pred exists in the nodes map
        if (!nodes.containsKey(pred)) {
            throw new NoSuchElementException("Node " + pred.toString() + " not found in the graph");
        }

        Node predNode = nodes.get(pred);
        // Log the nodes being searched for
        System.out.println("Searching for edge between nodes: " + pred.toString() + " and " + succ.toString());
        // search for edge through the predecessor's list of leaving edges
        for (Edge edge : predNode.edgesLeaving) {
            // Log the edge being checked
            System.out.println("Checking edge: " + edge.predecessor.data.toString() + " -> " + edge.successor.data.toString());
            // compare succ to the data in each leaving edge's successor
            if (edge.successor.data.equals(succ))
                return edge;
        }
        // Log that the edge was not found
